...
import { ListingProjectCard } from "./listing-card";
import { MOBILE_STATUS_CHIPS } from "./listing-utils";
  ...
  return (
    <div className="space-y-3 lg:hidden">
      ...
      <div className="flex gap-2 overflow-x-auto [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
        <button
          type="button"
          onClick={() => onChange({ status: "", paymentPlan: false })}
          className={cn(
            "shrink-0 rounded-full px-3 py-1.5 text-xs font-semibold",
            !filters.status && !filters.paymentPlan
        ...
        {MOBILE_STATUS_CHIPS.map((status) => (
          <button
            key={status}
            type="button"
            onClick={() =>
              onChange({
                status: filters.status === status ? "" : (status as ProjectStatus),
                paymentPlan: false,
              })
            }
            className={cn(
        ...
        <button
          type="button"
          onClick={() =>
            onChange({ paymentPlan: !filters.paymentPlan, status: "" })
          }
          className={cn(
            "shrink-0 rounded-full px-3 py-1.5 text-xs font-semibold",
            filters.paymentPlan
