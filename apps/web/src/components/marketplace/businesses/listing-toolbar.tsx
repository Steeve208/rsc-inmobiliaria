...
import { BUSINESS_CATEGORIES, POPULAR_BUSINESS_CITIES } from "./listing-utils";

type Props = {
  filters: NegociosFilters;
  view: NegociosView;
  onChange: (patch: Partial<NegociosFilters>) => void;
  onViewChange: (view: NegociosView) => void;
};

const SORTS: BusinessSort[] = ["newest", "price_asc", "price_desc", "revenue_desc"];
    ...
    <div className="space-y-3">
      <div className="hidden flex-wrap items-center gap-2 lg:flex">
        <FilterSelect
          ...
          value={filters.city}
          onChange={(city) => {
            const place = POPULAR_BUSINESS_CITIES.find((item) => item.city === city);
            onChange(
              city && place
                ? {
                    city: place.city,
        ...
        <FilterSelect
          label={t("category")}
          value={filters.type}
          onChange={(type) => onChange({ type: type as BusinessCategory | "" })}
          options={[
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="hidden items-center gap-2 lg:flex">
          <label className="inline-flex items-center gap-2 text-sm text-[#4B5563]">
            ...
            <select
              value={filters.sort}
              onChange={(event) => onChange({ sort: event.target.value as BusinessSort })}
    ...
    <div className="relative">
      <button
        ...
        className={cn(
          ...
          value ? "border-[#2BB8A8] text-[#0B1220]" : "border-[#E5E7EB] text-[#0B1220]",
        )}
      >
        {label}
        {value ? `: ${current}` : ""}
