...
export function LocationAutocomplete({
  ...
  return (
    <div ref={wrapRef} className={cn("relative min-w-0 flex-1", className)}>
      <div className="flex h-11 items-center gap-2 rounded-xl bg-white/[0.04] px-3 ring-1 ring-white/10 transition-shadow focus-within:ring-[#1d4ed8]/40">
        <MapPin className="size-4 shrink-0 text-[#60a5fa]" strokeWidth={1.75} />
        <input
          type="text"
          value={value}
        ...
        <button
          ...
          disabled={gpsLoading}
          title={t("useGps")}
          className="flex size-8 shrink-0 items-center justify-center rounded-lg text-[#60a5fa] transition-colors hover:bg-white/10 disabled:opacity-50"
          aria-label={t("useGps")}
        >
        ...
        <ul className="absolute left-0 right-0 top-[calc(100%+6px)] z-50 max-h-60 overflow-y-auto rounded-xl bg-[#0a1428]/95 py-1 shadow-2xl shadow-black/40 backdrop-blur-xl">
          {suggestions.map((item) => (
            <li key={`${item.lat}-${item.lng}-${item.label}`}>
              ...
              >
                <MapPin className="mt-0.5 size-3.5 shrink-0 text-[#60a5fa]" />
