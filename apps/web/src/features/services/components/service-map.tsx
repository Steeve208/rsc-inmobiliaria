...
function pinLabel(item: ServiceListing) {
  if (item.pricing === "quote" || item.price == null) return "·";
  return formatCompactMoney(item.price, item.currency);
}
...
export function ServiceMap({
  ...
  useEffect(() => {
    ...
    const mappable = items.filter((item) => isValidMapCoord(item.lng, item.lat));
  ...
  return <div ref={mapContainer} className={cn("overflow-hidden", className ?? "min-h-[360px]")} />;
}
